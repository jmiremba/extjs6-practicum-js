 ...
 * logged out.
 */
Ext.define('Practicum.SessionMonitor', {
  singleton: true,
  ...
  window: Ext.create('Ext.window.Window', {
    ...
    items: [{
      xtype: 'container',
      frame: true,
      html: "<p>You have been inactive for a while. To continue working, click the 'Continue Working' button.</p>"    
    },{
      xtype: 'label',
      text: ''
    }],
    buttons: [{
      ...
      handler: function() {
        Ext.TaskManager.stop(Practicum.SessionMonitor.countDownTask);
        Practicum.SessionMonitor.window.hide();
      ...
      handler: function() {
        ...
        Ext.ComponentQuery.query('button#logoutBtn')[0].fireEvent('click',
                Ext.ComponentQuery.query('button#logoutBtn')[0]);
      }
    }]
  }),
   ...
   */
  countDown: function() {
    this.window.down('label').update('You will be logged out in ' +  this.remaining +
                                     ' second' + ((this.remaining == 1) ? '.' : 's.') );
    --this.remaining;
    if (this.remaining < 0) {
